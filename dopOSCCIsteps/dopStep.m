...
try
    ...
    dop.step.col.stop = [.9 .6 .6]; % colouring the buttons at the ends of the steps
    dop.step.col.go = [.6 .9 .6];
    ...
    dop.step.action.string = {'Import','Channels','Heart','Events',...
        'Norm','Epoch','Screen','Baseline','LI','Plot','PlotSave','dop','Code','Close'}; %
    ...
    dop.step.action.tooltip = struct(...
        ...
        'dop','send ''dop'' variable to the workspace',...
    ...
    dop.tmp.li_num = find(ismember(dop.step.action.tag,'li')); % place holder to adjust position of extra buttons
    dop.tmp.n_extra = dop.step.action.n-dop.tmp.li_num;
    dop.tmp.x_left = .88;
    dop.tmp.y_top = .75;
    dop.tmp.y_bot = .15;
    dop.tmp.y_space = (dop.tmp.y_top-dop.tmp.y_bot)/dop.tmp.n_extra;
    dop.step.action.position(dop.tmp.li_num+1:end,:) = ...
        [repmat(dop.tmp.x_left,dop.tmp.n_extra,1),...
        abs(-dop.tmp.y_top:dop.tmp.y_space:-(dop.tmp.y_top-dop.tmp.y_space*(dop.tmp.n_extra-1)))'];
...
%     dop.step.action.position(end-1,:) = [.88 .45];
    dop.step.action.position(end,:) = [dop.tmp.x_left dop.tmp.y_bot];
    dop = rmfield(dop,'tmp');
