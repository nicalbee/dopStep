...
%     dop.tmp.last_action_num = dopStepLastActionButton(dop);
    ...
    % add 'finished' at the end'
    dop.step.steps(end-dop.tmp.last_action_num:end+1) = {'finished',dop.step.steps{end-dop.tmp.last_action_num:end}};
    
    if sum(ismember(dop.step.steps,'save'))
        dop.tmp.lan = find(ismember(dop.step.steps,'save')); %diff([find(ismember(dop.step.steps,'save')),numel(dop.step.steps)])-1;
    else
        dop.tmp.lan = find(ismember(dop.step.steps,'li')); %diff([find(ismember(dop.step.steps,'lI')),numel(dop.step.steps)])-1;
    end
    % add 'save_info' before save
%     dop.tmp.lan = dopStepLastActionButton(dop); % last action num
    dop.step.steps(dop.tmp.lan:end+1) = [{'save_info'},dop.step.steps(dop.tmp.lan:end)];
        ...
        switch dop.step.next.name
            ...
            case 'welcome'
                ...
                dop.step.next.string = {'That''s all there is for ''dopStep'' - an introduction to DOPOSCCI.',...
                    ...
                    'and/or ',...
                    'some of the code created (''code'' button) or the ''dopOSCCIexample.m'' ',...
            ...
            case 'save_info'
                ...
                dop.step.next.string = {'Save directory and variable description:',...
                    ['This step allows you to save various statistics to a data file. ',...
                    'Here you can set the data directory and read about ',...
                    'the output variable selection for the next page.'],...
                    'Directory:','type base directory','Browse',...
                     'Summaries: Allows you to specify whether the data is saved for a set or collection of epochs (ie ''overall'') and/or for each individual epoch (ie ''epoch'')',...
                     'Channels: Allows you to specify whether the data saved are from the Left, Right, Difference (left - right), and/or Average (mean of left & right) - Difference is the default and returns the laterality index',...
                     'Periods: Allows you to specify which periods the data saved are calculated from. Default is the period of interest (poi) which is used for the laterality index calculation but could also be the ''baseline'' period or entire ''epoch''',...
                     'Epochs: Allows you to specify the sets of epochs the data will be saved from. ''all'' has no screening, ''screen'' relates to all screening applied, ''odd'' and ''even'' relate to the odd and even numbered epochs, which are used for split-half reliability calculations.',...
                     'Variables: Allows you to specify which summary statistics will be saved for all of the above settings.',...
                    };
                dop.step.next.tag = {'title','info','save_dir_text','save_dir_edit','save_dir_browse',...
                    'Summaries_info','Channels_info','Periods_info','Epochs_info','Variables_info',...
                    };
                ...
                dop.step.next.position = [dop.step.next.title_position;... [dop.step.next.left_pos .87 .7 .1];
                    ...
                    dop.step.next.left_pos .5 .7 .1; ... % text {'Summary','overall','epoch'},...
                    dop.step.next.left_pos .4 .7 .1; ...
                    dop.step.next.left_pos .3 .7 .1; ...
                    dop.step.next.left_pos .2 .7 .1; ...
                    dop.step.next.left_pos .1 .7 .1; ...
                    ];
                dop.step.next.HorizontalAlignment = ...
                    [dop.step.next.HorizontalAlignment,...
                ...
                dop.step.next.Callback = {[],[],[],[],'dopStepBrowseFolder',[],[],[],[],[],[],[],[]};
            case 'save' % without directory input
                dop.step.next.style = {'text','text',...
                ...
                dop.step.next.position = [dop.step.next.title_position;... [dop.step.next.left_pos .87 .7 .1];
                    dop.step.next.info_position;... [dop.step.next.left_pos .7 .7 .1];
                    dop.step.next.left_pos .65 .1 .1; ... % text {'Summary','overall','epoch'},...
                    dop.step.next.left_pos+.1 .65 .1 .1; ... % checkbox
                    dop.step.next.left_pos+.2 .65 .1 .1; ... % checkbox
                    dop.step.next.left_pos .525 .1 .1; ... % text {'Channels','Left','Right','Difference','Average'}
                    dop.step.next.left_pos+.1 .525 .1 .1; ... % check
                    dop.step.next.left_pos+.2 .525 .1 .1; ... % check
                    dop.step.next.left_pos+.3 .525 .1 .1; ... % check
                    dop.step.next.left_pos+.4 .525 .1 .1; ... % check
                    dop.step.next.left_pos .4 .1 .1; ... % text {'Periods','epoch','base','poi'}
                    dop.step.next.left_pos+.1 .4 .1 .1; ... % check
                    dop.step.next.left_pos+.2 .4 .1 .1; ... % check
                    dop.step.next.left_pos+.3 .4 .1 .1; ... % check
                    dop.step.next.left_pos .275 .1 .1; ... % text {'Epochs','all','screen','odd','even'}
                    dop.step.next.left_pos+.1 .275 .1 .1; ... % check
                    dop.step.next.left_pos+.2 .275 .1 .1; ... % check
                    dop.step.next.left_pos+.3 .275 .1 .1; ... % check
                    dop.step.next.left_pos+.4 .275 .1 .1; ... % check
                ...
                dop.step.next.visible = ...
                    repmat({'on'},1,numel(dop.step.next.tag));
                dop.step.next.Callback = [{[],[]},...
                    repmat({'dopStepGetDef'},1,numel(dop.step.next.tag)-2)];
%                 case 'save' % with directory
%                 dop.step.next.style = {'text','text',...
%                     'edit','edit','pushbutton',...
%                     'edit','checkbox','checkbox',... % summary
%                     'edit','checkbox','checkbox','checkbox','checkbox',... % channels
