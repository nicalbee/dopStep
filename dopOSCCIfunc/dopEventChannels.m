...
try
    if okay
        ...
        if okay
            ...
            dop.tmp.event_index = find(strcmp(dop.data.channel_labels,'event'));
            dop.data.event(:,dop.tmp.event_index) = 0;
            
            for ii = 1 : 1 %numel(dop.tmp.event_index)
                dop.data.event(dop.event(ii).samples,dop.tmp.event_index(ii)) = 1;
                ...
                for i = 1 : numel(dop.event(ii).samples)
                    k = find(dop.data.event_plot(:,2) == dop.event(ii).samples(i),1,'first');
                ...
                for i = 1 : numel(dop.tmp.periods)
                    ...
                    for jjj = 1 : size(dop.tmp.(dop.tmp.prd),1)
                        ...
                        dop.data.event(:,dop.tmp.prd_column(ii)+i) = zeros(size(dop.data.event,1),1);
                        dop.data.channel_labels{dop.tmp.prd_column(ii)+i} = dop.tmp.prd_spec;
                        for j = 1 : numel(dop.event(ii).samples)
                            % patch is much easier
                            prd_sec = dop.event(ii).samples(j)*(1/dop.tmp.sample_rate) + dop.tmp.(dop.tmp.prd)(jjj,:);
                            ...
                            % regular data first
                            kpt = dop.event(ii).samples(j) + dop.tmp.(dop.tmp.prd)(jjj,:)/(1/dop.tmp.sample_rate);
                            ...
                            for jj = 1 : 2
                                ...
                                k = find(dop.data.([dop.tmp.prd_spec,'_plot'])(:,2) == dop.event(ii).samples(j));
