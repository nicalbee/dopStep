...
%                     {'data'};
        ...
        if okay
            % include all
            dop.epoch.manual = ones(1,dop.event(dop.tmp.screen_event).n);
            if ~isempty(dop.tmp.exclude) && isnumeric(dop.tmp.exclude) && ...
                    sum(dop.tmp.exclude <= dop.event(dop.tmp.screen_event).n) == numel(dop.tmp.exclude)
...
%                     ' (%u)\n\t(%s: %s)'],dop.tmp.exclude,dop.event(dop.tmp.screen_event).n,...
...
%             else
            ...
            if ~isempty(dop.tmp.exclude) && ...
                    sum(dop.tmp.exclude > dop.event(dop.tmp.screen_event).n)
                dop.tmp.greater = dop.tmp.exclude > dop.event(dop.tmp.screen_event).n;
                okay = 0;
                msg{end+1} = sprintf(['''exclude'' input variable (',...
                    dopVarType(dop.tmp.exclude),')',...
                    ...
                    ' \n\t(%s: %s)'],...
                    dop.tmp.exclude,sum(dop.tmp.greater),dop.event(dop.tmp.screen_event).n,...
                    mfilename,dop.tmp.file);
