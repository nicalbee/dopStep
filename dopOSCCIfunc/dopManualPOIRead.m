function [dop,okay,msg] = dopManualPOIRead(dop_input,varargin)
% dopOSCCI3: dopManualPOIRead
%
    ...
    if okay
        ...
        if okay
            ...
            switch dopInputCheck(dop_input)
                case 'dop'
                    if ~isempty(dop.tmp.poi_fullfile) && exist(dop.tmp.poi_fullfile,'file')
                        dop.tmp.import_data = importdata(dop.tmp.poi_fullfile);
                    elseif ~isempty(dop.tmp.poi_file) && ~isempty(dop.tmp.poi_dir) ...
                        ...
                        if exist('readtable','file')
                            dop.tmp.import_data = readtable(fullfile(dop.tmp.poi_dir,dop.tmp.poi_file),...
                                'delimiter',dop.tmp.delim);
                        else
                            dop.tmp.import_data = importdata(fullfile(dop.tmp.poi_dir,dop.tmp.poi_file));
                        end
                    else
                        okay = 0;
                ...
                case 'file'
                    %                     dop.tmp.poi_fullfile = dop_input;
                    if exist('readtable','file')
            ...
            if okay && ~isempty(dop.tmp.import_data)
                if exist('istable','file') && istable(dop.tmp.import_data)
                    dop.tmp.poi_list = table2cell(dop.tmp.import_data(:,1));
                    dop.tmp.poi_values = table2cell(dop.tmp.import_data(:,2));
                    % check for ~ or [] type of entries
                    for i = 1 : numel(dop.tmp.poi_values)
                        dop.tmp.poi_values{i} = checkManualString(dop.tmp.poi_values{i});
                    end
                else
                    dop.tmp.poi_list = cell(size(dop.tmp.import_data,1)-1,1); % minus 1 = assume header
                    dop.tmp.poi_values = cell(size(dop.tmp.import_data,1)-1,1); % minus 1 = assume header
                    for i = 2 : size(dop.tmp.import_data,1)
                        dop.tmp.row = textscan(dop.tmp.import_data{i},'%s%s','delimiter',dop.tmp.delim);
                        dop.tmp.poi_list{i-1} = char(dop.tmp.row{1});
                        dop.tmp.poi_values{i-1} = char(dop.tmp.row{2});
                        
                        %                     if ~isempty(strfind(dop.tmp.poi_values{i-1},'['));
                        %                         dop.tmp.poi_values{i-1} = eval(char(dop.tmp.row{2}));
                        %                     elseif ~isempty(sum(strfind(dop.tmp.poi_values{i-1},'~')));
                        %                         dop.tmp.tilda = strfind(dop.tmp.poi_values{i-1},'~');
                        ...
                        %                             end
                        %                         end
                        %                         dop.tmp.poi_values{i-1} = dop.tmp.epochs;
                        %                     end
                    end
                    

                end
                                    dop.poi.poi_list = dop.tmp.poi_list;
                    dop.poi.poi_values = dop.tmp.poi_values;
            else
                okay = 0;
                msg{end+1} = sprintf('Manual period of interest file is empty\n\t(%s: %s)',...
                    mfilename,dop.tmp.file);
                dopMessage(msg,dop.tmp.msg,1,okay,dop.tmp.wait_warn);
            end
        end
        switch dopInputCheck(dop_input)
            case 'dop'
                ...
                dop.msg = msg;
            case 'file'
                dop.poi_list = dop.tmp.poi_list;
                dop.poi_values = dop.tmp.poi_values;
        end
