...
try
    if okay
        ...
        inputs.defaults = struct(...
            ...
            'showmsg',1,...
            'wait_warn',0,...
            'act_change',5,... % change relative to mean/baseline
        ...
        if okay
            ...
            dop.epoch.change = ones(1,dop.tmp.n_epochs);
            
            % some descriptives
            dop.epoch.act_change_descriptives = {'mean','std','median','iqr','min','max'};
            for i = 1 : numel(dop.epoch.act_change_descriptives)
                dop.epoch.(['act_change_',dop.epoch.act_change_descriptives{i},'_ep']) = dop.epoch.change;
            end
                ...
                for j = 1 : dop.tmp.n_epochs
                    ...
                    switch dop.tmp.loop{jj}
                        case 'descriptives'
                            ...
                            for i = 1 : numel(dop.epoch.act_change_descriptives)
                                dop.epoch.(['act_change_',dop.epoch.act_change_descriptives{i},'_ep'])(j) = ...
                                    eval(sprintf('%s(abs(dop.tmp.diff))',dop.epoch.act_change_descriptives{i}));
                            end
                        ...
                        case 'exclusions'
                            ...
                            dop.epoch.change(j) = sum(dop.tmp.all) == numel(dop.tmp.diff);
            ...
            dop.epoch.change_note = sprintf(['logical variable denoting epochs',...
                ...
                dop.tmp.act_change_pct,dop.save.act_change_use,...
