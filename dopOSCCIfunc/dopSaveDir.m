...
try
    if okay
        ...
        %% main code
        if ~isempty(dop.tmp.base_save_dir)
            if ~exist(dop.tmp.base_save_dir,'dir')
                mkdir(dop.tmp.base_save_dir);
            end
            
            msg{end+1} = sprintf(['Base directory variable inputted '...
                ': %s'],dop.tmp.base_save_dir);
...
%         dop.tmp.base_save_dir = [dop_dir,'Data'];
        ...
        if dop.tmp.gui
            ...
            while 1
                dop.tmp.save_dir = fullfile(dop.tmp.base_save_dir,dop.tmp.task_name,dop.tmp.save_name);
        ...
        dop.save.save_dir = fullfile(dop.tmp.base_save_dir,dop.tmp.task_name,dop.tmp.save_name);%dop.tmp.save_dir;
        dop.def.save_dir = dop.save.save_dir;
