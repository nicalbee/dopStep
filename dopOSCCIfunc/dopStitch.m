...
%             {'epoch'};
        ...
        %% data check      
        % probably need something in here eventually...      
            ...
            if dop.stitch.match
                ...
                if jj == size(dop.stitch.file_sets,2)
                    ...
                    for ii = 1 : size(dop.stitch.file_sets,2)
                        dop.stitch.data = [dop.stitch.data,dop.stitch.collect(dop.stitch.match_column).data]; %(:,1:min(dop.stitch.epochs.n),:)];
                        ...
                        dop.tmp.filt = dop.stitch.epoch.screen(dop.stitch.match_column).data;
                        if dop.stitch.epoch.okay(dop.stitch.match_column) > min(dop.stitch.epoch.okay)
