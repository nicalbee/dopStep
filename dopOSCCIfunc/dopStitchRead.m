function [dop,okay,msg] = dopStitchRead(dop_input,varargin)
% dopOSCCI3: dopStitchRead
%
% [dop,okay,msg] = dopStitchRead(dop_input,[okay],[msg],...)
%
% notes:
...
%
% - 'stitch_file':
%   > e.g., dopFunction(dop_input,okay,msg,...,'stitch_file','C:\my_dir\stitch_use.dat',...)
...
%
% Created: 20-Jan-2016 NAB
% Edits:
% 20-Jan-2016 NAB getting this to work for Margriet quickly - will need
...
%         inputs.turnOn = {'overwrite'};
        ...
        inputs.defaults.column_labels = {'id','file_name1','file_name2'};
        %         inputs.required = ...
        %             {'file_list'};
        ...
        if okay
            ...
            switch dopInputCheck(dop_input)
                case 'dop'
                    if ~isempty(dop.tmp.stitch_fullfile) && exist(dop.tmp.stitch_fullfile,'file')

                    elseif ~isempty(dop.tmp.stitch_file) && ~isempty(dop.tmp.stitch_dir) ...
                            && exist(dop.tmp.stitch_dir,'dir') ...
                            && exist(fullfile(dop.tmp.stitch_dir,dop.tmp.stitch_file),'file')
                    ...
                    if okay
                        if ~isempty(strfind(dop.tmp.stitch_fullfile,'.csv'))
                            dop.tmp.delim = ',';
                        end
                        if exist('readtable','file')
                            dop.tmp.import_data = readtable(fullfile(dop.tmp.stitch_dir,dop.tmp.stitch_file),...
                                'delimiter',dop.tmp.delim);
                        else
                            dop.tmp.import_data = importdata(dop.tmp.stitch_fullfile);
                        end
                    end
                case 'file'
                    %                     dop.tmp.stitch_fullfile = dop_input;
                    if exist('readtable','file')
                        if ~isempty(strfind(dop_input,'.csv'))
                            dop.tmp.delim = ',';
                        end
            ...
            if okay && ~isempty(dop.tmp.import_data)
                if exist('istable','file') && istable(dop.tmp.import_data)
                    dop.tmp.stitch_table = dop.tmp.import_data;
                    dop.tmp.stitch_id = table2cell(dop.tmp.import_data(:,1));
                    dop.tmp.stitch_file_sets = table2cell(dop.tmp.import_data(:,2:end));
                    for i = 2 : size(dop.tmp.import_data,2)
                        dop.tmp.(sprintf('file_list%i',i-1)) = table2cell(dop.tmp.import_data(:,i));
                    end
%                     dop.tmp.stitch_values = table2cell(dop.tmp.import_data(:,2));
%                     % check for ~ or [] type of entries
%                     for i = 1 : numel(dop.tmp.stitch_values)
%                         dop.tmp.stitch_values{i} = checkManualString(dop.tmp.stitch_values{i});
%                     end
                else
                    okay = 0;
                msg{end+1} = sprintf('Not yet supported without MATLAB''s table functions\n\t(%s: %s)',...
                    mfilename,dop.tmp.file);
                dopMessage(msg,dop.tmp.msg,1,okay,dop.tmp.wait_warn);
                    
%                     dop.tmp.stitch_id = cell(size(dop.tmp.import_data,1)-1,1); % minus 1 = assume header
%                     dop.tmp.stitch_values = cell(size(dop.tmp.import_data,1)-1,1); % minus 1 = assume header
%                     for i = 2 : size(dop.tmp.import_data,1)
%                         dop.tmp.row = textscan(dop.tmp.import_data{i},'%s%s','delimiter',dop.tmp.delim);
%                         dop.tmp.stitch_list{i-1} = char(dop.tmp.row{1});
%                         dop.tmp.stitch_values{i-1} = char(dop.tmp.row{2});
%                         
%                        
%                     end
                ...
                dop.stitch.table = dop.tmp.stitch_table;
                dop.stitch.file_sets = dop.tmp.stitch_file_sets;
            else
                okay = 0;
                msg{end+1} = sprintf('Stitch file is empty\n\t(%s: %s)',...
                    mfilename,dop.tmp.file);
                dopMessage(msg,dop.tmp.msg,1,okay,dop.tmp.wait_warn);
            end
        end
        switch dopInputCheck(dop_input)
            case 'dop'
                ...
                dop.msg = msg;
            case 'file'
                dop.stitch_list = dop.tmp.stitch_list;
                dop.stitch_values = dop.tmp.stitch_values;
        end
